...
genome_t * genome_load_chromosome(genome_t * genome, char * chromosome_name, int chrom_index) {
	...
	//printf("LOADING %s\n", chromosome_name);
	genome->chromosome->index = chrom_index;
	int size;
	size = genome->info->sizes[chrom_index];
